...
- **카드 설정** 모달에서 이미지를 **드래그앤드롭** 또는 **파일 선택** 버튼으로 올리면 → `POST /api/upload`로 파일 전송 → Blob에 업로드 → 반환된 **URL**을 카드의 전면/후면 이미지로 저장
- **이미지 URL**을 직접 입력하면 → 그대로 URL만 저장 (업로드 없음)
- 기존에 **data URL(base64)** 로 저장된 카드도 그대로 표시됨
- `BLOB_READ_WRITE_TOKEN`이 없으면 업로드 시 503과 함께 "Blob 스토어를 생성하세요" 안내가 뜸
- **카드 삭제** 시 해당 카드의 전면/후면 이미지가 Blob URL이면 Blob에서도 함께 삭제됨 (용량 절약)
